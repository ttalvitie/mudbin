...
impl Qemu {
    pub fn wait(self) -> BoxFuture<()> {
        ...
        self.process
            ...
            .and_then(move |()| future::result(workdir.close().chain_err(|| "Closing temporary directory for QEMU failed")))
            .into_box()
    }
}
...
impl QemuConfig {
    pub fn new() -> Result<QemuConfig> {
        let workdir = TempDir::new("mudbin_tmp").chain_err(|| "Creating temporary directory for QEMU failed")?;
        ...
        Ok(QemuConfig{command, workdir, next_binding_idx: 0})
    }
    ...
    fn bind_file<P: AsRef<Path>>(&mut self, src: P) -> Result<String> {
        ...
        let src = src.canonicalize().chain_err(|| "Could not canonicalize QEMU binding source file path")?;
        let binding = self.alloc_binding();
        symlink(src, self.workdir.path().join(&binding)).chain_err(|| "Could not create symlink for QEMU file binding")?;
        Ok(binding)
    }

    pub fn boot_kernel<P: AsRef<Path>, Q: AsRef<Path>>(mut self, kernel: P, initrd: Q) -> Result<Self> {
    ...
    pub fn spawn(mut self) -> Result<Qemu> {
        ...
        Ok(Qemu { process, workdir: self.workdir })
    }
}
